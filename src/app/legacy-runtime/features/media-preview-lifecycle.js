import { prepareVideoThumbnails } from './video-thumbnail-lifecycle.js';
...
export function createMediaPreviewLifecycle({
    ...
    const openMediaPreview = (media, sourceElement = null) => {
        ...
        overlay.innerHTML = `
            <button type="button" class="media-lightbox-close" aria-label="${escapeHTML(closePreviewLabel)}"><svg viewBox="0 0 24 24" fill="none" stroke="#ffffff" stroke-width="2.4" stroke-linecap="round" aria-hidden="true"><path d="M6 6l12 12M18 6 6 18"/></svg></button>
    ...
    const bindMediaPreviewButtons = (root, mediaParts = []) => {
        prepareVideoThumbnails(root, { document });

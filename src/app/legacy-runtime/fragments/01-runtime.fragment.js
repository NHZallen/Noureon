    ...
    document.getElementById('current-model-btn').addEventListener('click', () => {
        ...
        const bindMediaPreviewButtons = (root, mediaParts = []) => {
            ...
            const clearMediaCopyPopover = () => {
                document.querySelector('.media-copy-popover')?.remove();
                document.querySelectorAll('.media-longpress-active').forEach(el => el.classList.remove('media-longpress-active'));
                pressedButton?.classList.remove('media-longpress-active');
                pressedButton = null;
            };
            const getClipboardBlob = async (src, mimeType) => {
                const originalBlob = await (await fetch(src)).blob();
                if (!mimeType.startsWith('image/') || mimeType === 'image/png') return originalBlob;
                try {
                    ...
                    console.warn('Image clipboard conversion failed:', error);
                    return originalBlob;
                }
            };
            const copyMediaToClipboard = async (media) => {
                if (!media) return;
                const mimeType = media.mimeType || 'application/octet-stream';
                const src = getInlineMediaSrc(media);
                const copyText = config.uiLanguage === 'en' ? 'Copied' : '已複製';
                const fallbackText = config.uiLanguage === 'en' ? 'Media data copied' : '媒體資料已複製';
                try {
                    const blob = await getClipboardBlob(src, mimeType);
                    if (navigator.clipboard?.write && window.ClipboardItem) {
                        await navigator.clipboard.write([new ClipboardItem({ [blob.type || mimeType]: blob })]);
                        showNotification(copyText, 'success');
                        return;
                    }
                    if (navigator.clipboard?.writeText) {
                        await navigator.clipboard.writeText(src);
                        showNotification(fallbackText, 'success');
                        return;
                    }
                    throw new Error('Clipboard API unavailable');
                } catch (error) {
                    console.warn('Media copy failed:', error);
                    if (navigator.clipboard?.writeText) {
                        try {
                            await navigator.clipboard.writeText(src);
                            showNotification(fallbackText, 'success');
                            ...
                            console.warn('Media copy fallback failed:', fallbackError);
                        }
                    }
                    showNotification(config.uiLanguage === 'en' ? 'This browser cannot copy this media.' : '此瀏覽器無法複製此媒體。', 'warning');
                }
            };
            const showMediaCopyPopover = (button, media) => {
                clearMediaCopyPopover();
                pressedButton = button;
                button.classList.add('media-longpress-active');
                const popover = document.createElement('div');
                popover.className = 'media-copy-popover';
                popover.innerHTML = `
                    <button type="button" class="media-copy-popover-btn">
                        ${config.uiLanguage === 'en' ? 'Copy' : '複製'}
                    </button>
                `;
                document.body.appendChild(popover);
                ...
                requestAnimationFrame(() => popover.classList.add('visible'));
                popover.querySelector('.media-copy-popover-btn')?.addEventListener('click', async (event) => {
                    event.stopPropagation();
            ...
            root.querySelectorAll('.message-media-thumb').forEach(button => {
                const cancelPress = () => {
                    ...
                    if (!document.querySelector('.media-copy-popover')) {
                        button.classList.remove('media-longpress-active');
                    }
                };
                button.addEventListener('pointerdown', (event) => {
                    if (event.pointerType === 'mouse' && event.button !== 0) return;
                    const mediaIndex = Number(button.dataset.mediaIndex);
                    const media = mediaParts[mediaIndex];
                    const mimeType = media?.mimeType || '';
                    if (!mimeType.match(/^(image|video)\//)) return;
                    clearMediaCopyPopover();
                    pressedButton = button;
                    button.classList.add('media-longpress-active');
                    pressTimer = setTimeout(() => {
                        suppressClick = true;
                        showMediaCopyPopover(button, media);
                    }, 520);
                });

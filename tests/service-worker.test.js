...
async function createHarness({
  ...
  cacheNames = [],
  currentCacheMatches = new Map([['/__noureon-shell-ready-v23__', createResponse('precache-shell')]])
} = {}) {
...
test('activation removes only stale Noureon caches and notifies open clients', async () => {
  const harness = await createHarness({
    cacheNames: ['noureon-cache-v21', 'noureon-cache-v22', 'noureon-cache-v23', 'another-app-cache']
  });
  const completion = [];
  ...
  assert.deepEqual(harness.cacheDeletes, ['noureon-cache-v21']);
...
test('activation retains the previous Noureon cache when the new shell was not precached', async () => {
  const harness = await createHarness({
    cacheNames: ['noureon-cache-v22', 'noureon-cache-v23'],
...
test('install precaches every manifest code asset before activating the new worker', async () => {
  ...
  assert.ok(harness.cachePuts.some(([key]) => key === '/__noureon-shell-ready-v23__'));
});

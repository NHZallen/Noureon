...
async function createHarness({
  ...
  cacheNames = [],
  currentCacheMatches = new Map([['/__noureon-shell-ready-v24__', createResponse('precache-shell')]])
} = {}) {
...
test('activation removes only stale Noureon caches and notifies open clients', async () => {
  const harness = await createHarness({
    cacheNames: ['noureon-cache-v21', 'noureon-cache-v22', 'noureon-cache-v23', 'noureon-cache-v24', 'another-app-cache']
  });
  const completion = [];
  ...
  assert.deepEqual(harness.cacheDeletes, ['noureon-cache-v21', 'noureon-cache-v22']);
...
test('activation retains the previous Noureon cache when the new shell was not precached', async () => {
  const harness = await createHarness({
    cacheNames: ['noureon-cache-v23', 'noureon-cache-v24'],

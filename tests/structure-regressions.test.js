...
test('general message markup rendering is isolated from the 01 runtime DOM shell', async () => {
  ...
  assert.match(sidebarChatAstraRenderSource, /\{\s*addMessageToUI,\s*isActiveConversationViewCurrent,\s*refreshMessageHistorySources,\s*renderChat\s*\}\s*=\s*createMessageListLifecycle\(\{/);

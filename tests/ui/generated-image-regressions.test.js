...
test('targeted editor exposes history controls, thickness preview, and complete mobile image sizing', () => {
  ...
  assert.match(css, /@media\s*\(max-width:\s*640px\)[\s\S]*\.generated-image-editor-canvas-wrap,\s*[\r\n\s]*\.generated-image-editor-photo\s*\{[\s\S]*max-height:\s*max\(18rem,\s*calc\(100dvh\s*-\s*15\.25rem\)\);/s);
  ...
  assert.match(css, /\.generated-image-editor-size-preview::before[^{]*\{[\s\S]*width:\s*min\(var\(--brush-size/s);

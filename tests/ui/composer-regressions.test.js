...
test('desktop active mode and Astras pills swap their leading icon to the themed close icon on hover', () => {
  ...
  assert.match(css, /#input-indicator-container\s+#close-search-btn-input\s+svg,[\s\S]*#input-indicator-container\s+#close-astras-btn-input\s+svg\s+\*[^{]*\{[^}]*color:\s*var\(--button-primary-bg\)\s*!important;[^}]*stroke:\s*currentColor\s*!important;/s);

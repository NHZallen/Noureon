...
test('model council manager uses compact pills and a bounded scroll area', () => {
  ...
  assert.match(css, /\.council-mode-tabs[^{]*\{[^}]*height:\s*2\.15rem;[^}]*box-shadow:\s*none;/s);
  assert.match(css, /\.council-mode-tabs button\.active[^{]*\{[^}]*border-color:\s*var\(--border-color\)\s!important;[^}]*background:\s*#ffffff\s!important;[^}]*color:\s*var\(--button-primary-bg\)\s!important;[^}]*box-shadow:\s*none;/s);

...
test('model council manager uses compact pills and a bounded scroll area', () => {
  ...
  assert.match(css, /\.council-mode-tabs button\.active[^{]*\{[^}]*border-color:\s*color-mix\(in srgb,\s*var\(--button-primary-bg\)\s*34%,\s*var\(--border-color\)\)\s!important;[^}]*background:\s*#ffffff\s!important;[^}]*color:\s*var\(--button-primary-bg\)\s!important;/s);

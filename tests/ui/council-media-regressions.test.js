...
test('model council manager uses compact pills and a bounded scroll area', () => {
  ...
  assert.match(css, /\.council-mode-tabs button\.active[^{]*\{[^}]*border-color:\s*var\(--button-primary-bg\)\s!important;[^}]*background:\s*var\(--button-primary-bg\)\s!important;[^}]*color:\s*var\(--button-primary-text\)\s!important;/s);

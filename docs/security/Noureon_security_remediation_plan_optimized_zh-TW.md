...
> 實作狀態（2026-07-12，依產品決策調整）：使用者要求 API Key 在重整與重開瀏覽器後仍保留，因此未採 session-only。現行設計以 IndexedDB 保存非匯出 AES-GCM `CryptoKey` 與獨立密文，舊明文資料會一次性加密遷移後刪除。此方案避免儲存層直接出現明文，但無法抵禦同源 XSS 或已控制的瀏覽器環境；UI 與隱私說明必須明確揭露此限制。
